javascript:(function(){function callback(){(function($){var jQuery=$;alert("Ready to run\nAuthor: Nofan Rambe");
...
function checkClaimButton() {
  ...
  if (claimButton) {
    ...
    console.log(
      'Claim button clicked. Author: Nofan Rambe'
    );
...
processElements();})(jQuery.noConflict(true))}var s=document.createElement("script");s.src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js";if(s.addEventListener){s.addEventListener("load",callback,false)}else if(s.readyState){s.onreadystatechange=callback}document.body.appendChild(s);})()
